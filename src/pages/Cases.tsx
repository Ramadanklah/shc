  ...
  return (
    <main className="min-h-[70vh] bg-white py-16">
      <div className="container mx-auto px-4">
        <h1 className="text-3xl font-bold text-syria-dark mb-8">Medical Cases</h1>
            ...
            return (
              <Card key={c.id} className="p-6 shadow flex flex-col">
                ...
                <div className="flex justify-between text-sm mb-2">
                  <span className="font-semibold">${c.amountRaised.toLocaleString()} raised</span>
                  <span className="text-gray-500">of ${c.goalAmount.toLocaleString()}</span>
                </div>
                <Button asChild className="mt-auto bg-syria-teal hover:bg-syria-teal-dark">
                  <Link to={`/cases/${c.id}`}>View Details</Link>
                </Button>
              </Card>
            );
          })}
